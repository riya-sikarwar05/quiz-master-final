...
def send_login_reminder(subject, body):
    try:
        ...
        today = date.today()
        not_login_today = [Users.Email for Users in Users.query.filter(
            Users.Email.isnot(None),
            Users.lastLogin != today
        ).all()]
...
def send_pdf_to_multiple_users(pdf_path, subject, body):
    try:
        ...
        recipient_emails = [Users.Email for Users in Users.query.filter(Users.Email.isnot(None)).all()]
        sender_email = 'putYourEmailHere@gmail.com'
        sender_password = 'putYourPasswordhere'
